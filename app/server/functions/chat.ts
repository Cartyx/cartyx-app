...
export const listMessages = withLogging(
  'chat.listMessages',
  async ({ data }: { data: z.infer<typeof listMessagesSchema> }) => {
    try {
...
export const saveMessage = withLogging(
  'chat.saveMessage',
  async ({ data }: { data: z.infer<typeof saveMessageSchema> }) => {
    try {
      const user = await getSession();
      if (!user) throw new Error('Not authenticated');
