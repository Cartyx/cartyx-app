...
export function PlayerCard({ player, onClick, onEdit, onDelete }: PlayerCardProps) {
  ...
  return (
    <div
      className={`group relative w-full border-b border-white/[0.05] hover:bg-white/[0.03] transition-colors${isDeceased ? ' opacity-70' : ''}`}
      style={
        isDeceased
          ? {
              borderLeftWidth: 4,
    ...
    >
      <div
        ...
        className="flex items-center gap-3 w-full px-4 py-3 text-left cursor-grab active:cursor-grabbing"
      >
        {/* Avatar */}
        <div
