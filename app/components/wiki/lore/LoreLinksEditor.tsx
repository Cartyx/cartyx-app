...
const KIND_LABELS: Record<LoreLinkKind, string> = {
  character: 'Character',
  player: 'Player',
  location: 'Location',
  race: 'Race',
...
const KIND_BADGE_COLORS: Record<LoreLinkKind, string> = {
...
interface Props {
  campaignId: string;
  links: LoreLink[];
  onChange: (links: LoreLink[]) => void;
  disabled?: boolean;
}

export function LoreLinksEditor({ campaignId, links, onChange, disabled }: Props) {
  const uid = useId();
  const kindSelectId = `lore-links-kind-${uid}`;
  const entitySelectId = `lore-links-entity-${uid}`;

  const [kind, setKind] = useState<LoreLinkKind>('character');
  const [selectedId, setSelectedId] = useState('');

  // Fetch all four entity lists — always enabled so the selects populate quickly
  ...
  }, [kind, characters, players, locations, races]);

  // Reset entity selection when kind changes
  const handleKindChange = (newKind: LoreLinkKind) => {
    setKind(newKind);
    setSelectedId('');
  };
  ...
  const handleAddLink = () => {
    ...
    const newLink: LoreLink = { kind, id: selectedId, label: candidate.label };
    onChange([...links, newLink]);
    setSelectedId('');
  };

  const handleRemoveLink = (removeKind: LoreLinkKind, removeId: string) => {
    onChange(links.filter((l) => !(l.kind === removeKind && l.id === removeId)));
  };

  return (
    <div data-testid="lore-links-editor" className="space-y-3">
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="flex-shrink-0">
            ...
            <select
              id={kindSelectId}
              value={kind}
              onChange={(e) => handleKindChange(e.target.value as LoreLinkKind)}
            ...
            >
              {(Object.keys(KIND_LABELS) as LoreLinkKind[]).map((k) => (
