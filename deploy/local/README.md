...
| Pod stuck `ImagePullBackOff`                         | kind didn't get the image. Re-run `./deploy/local/deploy-kind.sh` (it rebuilds and `kind load`s); the chart uses `pullPolicy: Never` so the image must be side-loaded. |
